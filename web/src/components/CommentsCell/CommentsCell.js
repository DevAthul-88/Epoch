...
export const Success = ({ commentsPage, totalRating, totalComments, videoId }) => {
  const { classes } = useStyles()
  const [loading, setLoading] = useState(false)
  const { isAuthenticated, currentUser } = useAuth()
  const rating = commentsPage.comments.map((e) => {
    return e.rating
  })
  ...
  return (
    <>
      ...
      <LoadingOverlay visible={loading} overlayBlur={2} />
        ...
        {commentsPage.comments.map((item) => {
          return (
            <Paper
              key={item.id}
              mt="lg"
     ...
     <Pagination count={commentsPage.count} route={`watch/${videoId}`} />
    </>
  )
}
